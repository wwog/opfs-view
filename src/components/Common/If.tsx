...
/**
 * @description 当condition为true时，渲染children,不使用也可以,使用可以方便对齐并整理
 * @param props 
 * @returns 
 */
...
//#region component
/**
 * 当condition为true时，渲染children,内部可以包含多个ElseIf和一个Else
 */
export const If: React.FC<IfProps> & {
  ...
  React.Children.forEach(children, (child) => {
    //如果是string, isVaildElement会返回false,但是我们也需要显示root下的文本
    if (React.isValidElement(child)) {
      if (child.type && (child.type as typeof ElseIf).displayName === 'ElseIf') {
  ...
  if (elseComponent) {
    return <>{(elseComponent as React.ReactElement<ElseProps>).props.children}</>
  }
